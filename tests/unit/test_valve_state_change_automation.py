...
class TestValveStateChangeAutomation:
    ...
    async def test_setup_registers_listeners(self, automation, mock_redis_client):
        """Test that setup registers listeners for all valve switches."""
        with patch(
    ...
    async def test_update_valve_state_no_matching_zone(
        ...
        # Reset the mock to return zones with different valve switches
        mock_redis_client.get_zone_ids = AsyncMock(return_value=["zone1"])
        mock_redis_client.get_zone_state = AsyncMock(
            return_value={
                "id": "zone1",
                "valve_switch_entity_id": "switch.different_valve",
                "valve_state": "closed",
            }
        )
    ...
    async def test_stop_cancels_listeners(self, automation):
        """Test that stop() cancels all listeners."""
